...
final class ConfigWebServiceRequest {
    ...
    static func execute(accountAddress: String, success: @escaping ([String: String]) -> Void, failure: @escaping (String?) -> Void) {
        ...
        WebServicesBaseRequest().executeRequest(url: url, method: .get, params: nil, headers: headers, success: { result in
            guard let data = result["data"] as? [String: String] else { failure(nil); return }
            success(data)
        }, failure: failure)
    }
}
